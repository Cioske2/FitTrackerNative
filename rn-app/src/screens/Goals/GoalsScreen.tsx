    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={{ paddingBottom: 0 }}>
        <View style={wrapStyle}>
          ...
          {/* Header with Back Button and Pill Add Button */}
          <View style={styles.headerBar}>
            <Pressable onPress={() => navigation.goBack()} style={styles.headerLeft}>
              <Ionicons name="chevron-back" size={28} color={colors.textPrimary} />
            </Pressable>
            <View style={styles.headerCenter}>
              <Text style={styles.headerTitle}>Obiettivi</Text>
            </View>
            <View style={styles.headerRight}>
              <Pressable onPress={() => setAddModal(true)} style={styles.addBtnPill}>
                <Text style={styles.addBtnPillTxt}>＋</Text>
              </Pressable>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  // New Header Styles
  headerBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, height: 44 },
  headerLeft: { width: 60, alignItems: 'flex-start' },
  headerCenter: { flex: 1, alignItems: 'center' },
  headerRight: { width: 60, alignItems: 'flex-end' },
  headerTitle: { fontSize: 20, fontWeight: '700', color: colors.textPrimary },
  addBtnPill: { backgroundColor: colors.accent, paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20 },
  addBtnPillTxt: { color: '#fff', fontSize: 18, fontWeight: '600' },
