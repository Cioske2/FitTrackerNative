...
export default function WorkoutScreen() {
  ...
  const groupedToShow = useMemo(
    () => (showAllWorkouts ? grouped : grouped.slice(0, 7)),
    [grouped, showAllWorkouts],
  );

  const exerciseNames = useMemo(
    () => Array.from(new Set(workouts.map((w) => w.exerciseName))).sort(),
    [workouts],
  );
  ...
  const series: Series = useMemo(
    () => buildExerciseSeries(workouts, selectedExercise),
  ...
  const addCurrentToPending = async () => {
    const repsInfo = normalizeRepsInput(currentExercise.reps);
    const derivedSets = currentExercise.sets
      ? Number(currentExercise.sets)
      : repsInfo.repsDetail
        ? repsInfo.repsArray.length
        : 0;
    if (!currentExercise.exerciseName || !derivedSets || !repsInfo.reps) {
      Alert.alert(
        "Mancano dati",
        "Inserisci almeno nome, serie e ripetizioni.",
      );
      return;
    }
    ...
    const exerciseLibraryService = (
      await import("../../services/exerciseLibraryService")
    ).default;
    const normalizedName = await exerciseLibraryService.findBestMatch(
    ...
    setPendingExercises([
      ...pendingExercises,
      {
        ...
        sets: derivedSets,
        reps: repsInfo.reps,
        repsDetail: repsInfo.repsDetail || "",
  ...
  const submitAll = async () => {
    ...
    if (currentExercise.exerciseName && currentExercise.reps) {
    ...
    try {
      ...
      for (const ex of list) {
        const repsInfo = normalizeRepsInput(String(ex.reps ?? ""));
        const repsDetail = ex.repsDetail || repsInfo.repsDetail || "";
        const repsDetailInfo = repsDetail
          ? normalizeRepsInput(repsDetail)
          : { repsArray: [] as number[] };
        const derivedSets = Number(ex.sets) ||
          (repsDetail ? repsDetailInfo.repsArray.length : 0);
        const notes = buildNotesWithRepsDetail(ex.notes || "", repsDetail);
        await add({
          exerciseName: ex.exerciseName.trim(),
          sets: derivedSets,
          reps: Number.isFinite(Number(ex.reps)) ? Number(ex.reps) : repsInfo.reps,
  ...
  const capitalizedMonth = showAll
    ? "Tutti"
  ...
  return (
    ...
    >
      ...
      {!loading && grouped.length === 0 && (
        <Text style={styles.empty}>Nessun allenamento in questo mese</Text>
      )}
      ...
      {hasMore && (
        ...
        >
          <Text style={styles.showMoreTxt}>
            {loadingMore ? "Caricamento..." : "Carica altri allenamenti"}
          </Text>
        </Pressable>
      )}
        ...
        >
          <View style={styles.exerciseChipsRow}>
            {exerciseNames.map((n) => (
              <Pressable
                key={n}
                onPress={() => setSelectedExercise(n)}
                style={[
                  styles.exerciseChip,
                  selectedExercise === n && styles.exerciseChipActive,
                ]}
              >
                <Text
                  style={[
                    styles.exerciseChipTxt,
                    selectedExercise === n && styles.exerciseChipTxtActive,
                  ]}
                >
                  {n}
                </Text>
            ...
            {exerciseNames.length === 0 && (
              <Text style={styles.noExercises}>Aggiungi un allenamento</Text>
            )}
          </View>
        </ScrollView>
        <View style={{ width: "100%" }}>
          <LineChart
            ...
            width={Math.max(
              ...
              Math.max(series.points.length, 6) * 42,
            )}
            height={200}
            withInnerLines={true}
            withOuterLines={true}
      ...
      <Modal
        ...
        animationType="fade"
        transparent
        onRequestClose={() => setSelectionModal(false)}
      >
        <Pressable
          ...
          <View style={styles.selectionCard}>
            ...
            >
              ...
              <View style={{ marginLeft: 12 }}>
                ...
                <Text style={styles.selectionOptionSub}>
                  Inserisci esercizi, serie e ripetizioni
                </Text>
              </View>
              <Ionicons
            ...
            >
              ...
              <View style={{ marginLeft: 12 }}>
                ...
                <Text style={styles.selectionOptionSub}>
                  Incolla testo o scrivi in linguaggio naturale
                </Text>
              </View>
              <Ionicons
                name="chevron-forward"
          ...
          >
            <View style={styles.modalCard}>
              ...
              <ScrollView style={{ maxHeight: 150, marginBottom: 10 }}>
                  ...
                  <View key={i} style={styles.pendingRow}>
                    <Text style={styles.pendingText}>
                      ...
                      {formatRepsLabel(
                        p.sets,
                        p.reps,
                        p.repsDetail || extractRepsDetail(p.notes),
                      )}{" "}
                      {p.weight ? `@ ${p.weight}kg` : ""}
                    </Text>
                    <Pressable onPress={() => removePending(p.id)}>
                      <Ionicons
                        name="trash-outline"
                        size={18}
                        color="#ef4444"
                ...
                {pendingExercises.length === 0 && (
                  <Text style={styles.emptyPending}>
                    Nessun esercizio aggiunto alla lista
                  </Text>
                )}
              </ScrollView>
              ...
              <TextInput
                ...
                onChangeText={(t) =>
                  setCurrentExercise((f) => ({ ...f, exerciseName: t }))
                }
              />
              <View style={styles.rowInputs}>
                <TextInput
                  ...
                  onChangeText={(t) =>
                    setCurrentExercise((f) => ({ ...f, sets: t }))
                  }
                />
                <TextInput
                  placeholder="Ripet. (es. 8/6/5)"
                  placeholderTextColor={colors.textMuted}
                  keyboardType="numbers-and-punctuation"
                  style={[styles.input, styles.inputSmall]}
                  value={currentExercise.reps}
                  onChangeText={(t) =>
                    setCurrentExercise((f) => ({ ...f, reps: t }))
                  }
                />
                <TextInput
                  placeholder="Peso"
                  ...
                  onChangeText={(t) =>
                    setCurrentExercise((f) => ({ ...f, weight: t }))
                  }
                />
              </View>
              ...
              <View style={styles.modalBtnsRow}>
                ...
                >
                  <Text style={styles.btnPrimaryTxt}>
                    {submitting
                      ? "Salvataggio..."
                      : `Salva Tutto (${pendingExercises.length + (currentExercise.exerciseName ? 1 : 0)})`}
                  </Text>
                </Pressable>
              </View>
            </View>
      ...
      <Modal
        ...
        transparent
        onRequestClose={() => {
          if (!parsing) setParseModal(false);
        }}
      >
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView
...
Curl manubri 3x12 12kg`}
              ...
              <View
                style={{
                  flexDirection: "row",
                  gap: 12,
                  marginBottom: parsed.length ? 12 : 4,
                }}
              >
                {/* Swap: Chiudi first, Analizza second */}
                <Pressable
                  style={[styles.btn, styles.btnCancel]}
                  onPress={() => !parsing && setParseModal(false)}
                >
                  <Text style={styles.btnTxt}>Chiudi</Text>
                </Pressable>
                <Pressable
                  style={[
                    styles.btn,
                    styles.btnSecondary,
                    (parsing || !parseText.trim()) && styles.btnDisabled,
                  ]}
                  ...
                  onPress={async () => {
                    ...
                    setParsed([]);
                    try {
                      const res = await workoutParser.parseWorkout(
                        parseText.trim(),
                      );
                      setParsed(res);
                      if (res.length === 0)
                        Alert.alert(
                          "Nessun esercizio",
                          "Non sono stati riconosciuti esercizi validi.",
                        );
                    } catch (e: any) {
                      Alert.alert(
                        "Errore parsing",
                        e.message || "Impossibile parsificare",
                      );
                    } finally {
                      setParsing(false);
                    }
              ...
              {parsed.length > 0 && (
                <Pressable
                  style={[
                    styles.btn,
                    styles.btnPrimary,
                    savingParsed && styles.btnDisabled,
                  ]}
                    ...
                    try {
                      for (const p of parsed) {
                        await add({
                          ...
                          notes: buildNotesWithRepsDetail(
                            p.notes || "",
                            extractRepsDetail(p.notes),
                          ),
                        });
                ...
                >
                  {savingParsed ? (
                    ...
                    <Text
                      style={styles.btnPrimaryTxt}
                    >{`Salva ${parsed.length}`}</Text>
                  )}
                </Pressable>
              )}
            </View>
          ...
          <View style={styles.dayModalCard}>
            ...
            <ScrollView style={{ maxHeight: 400 }}>
              ...
              {dayModal?.items.length === 0 && (
                <Text style={styles.empty}>Vuoto</Text>
              )}
            </ScrollView>
          </View>
        </View>
      ...
      {/* Month Picker Modal */}
      <Modal
        visible={monthPickerModal}
        animationType="fade"
        transparent
        onRequestClose={() => setMonthPickerModal(false)}
      >
        <Pressable
          style={styles.modalOverlay}
          onPress={() => setMonthPickerModal(false)}
        >
          <View style={styles.monthPickerCard}>
            ...
            <ScrollView style={{ maxHeight: 400 }}>
              <Pressable
                style={[
                  styles.monthOption,
                  showAll && styles.monthOptionActive,
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    showAll && styles.monthOptionTextActive,
                  ]}
                >
                  Tutti
                </Text>
...
function normalizeRepsInput(repsText: string) {
  const trimmed = repsText.trim();
  if (!trimmed) return { reps: 0, repsDetail: "", repsArray: [] as number[] };
  const numeric = Number(trimmed);
  if (!Number.isNaN(numeric))
    return { reps: numeric, repsDetail: "", repsArray: [numeric] };
  const repsArray = trimmed
    .split(/[^0-9]+/)
    .filter(Boolean)
    .map((n) => parseInt(n, 10))
  ...
  if (repsArray.length >= 2) {
    ...
    return { reps: avg, repsDetail: repsArray.join("/"), repsArray };
  }
  const fallback = parseInt(trimmed, 10);
  return {
    reps: Number.isNaN(fallback) ? 0 : fallback,
    repsDetail: "",
    repsArray: Number.isNaN(fallback) ? [] : [fallback],
  };
}

function extractRepsDetail(notes?: string) {
  if (!notes) return "";
  const match = notes.match(/reps_detail\s*:\s*([0-9/,.\-\s]+)/i);
  if (!match) return "";
  return match[1]
    .trim()
    .replace(/[,.\s-]+/g, "/")
...
function buildNotesWithRepsDetail(notes: string, repsDetail: string) {
  const cleaned = notes
    ? notes.replace(/\s*\|?\s*reps_detail\s*:\s*[0-9/,.\-\s]+/gi, "")
        .trim()
    : "";
  if (!repsDetail) return cleaned;
  return cleaned ? `${cleaned} | reps_detail:${repsDetail}` : `reps_detail:${repsDetail}`;
}

function formatRepsLabel(sets: number, reps: number, repsDetail?: string) {
  if (repsDetail) return `${sets}x${repsDetail}`;
  return `${sets}x${reps}`;
}
...
function buildExerciseSeries(workouts: any[], exercise: string | null): Series {
  ...
  let step = 1;
  if (rawLabels.length > 36) step = 6;
  else if (rawLabels.length > 30) step = 5;
  else if (rawLabels.length > 24) step = 4;
  else if (rawLabels.length > 18) step = 3;
  else if (rawLabels.length > 12) step = 2;
