...
export default function FoodDiaryScreen({ navigation }: Props) {
  ...
  const [manualForm, setManualForm] = useState({ name: '', quantity: '100', unit: 'g', calories: '', protein: '', carbs: '', fat: '' });
  const [manualSubmitting, setManualSubmitting] = useState(false);

  const submitManual = async () => {
    if (!manualForm.name.trim()) { Alert.alert('Errore', 'Nome richiesto'); return; }
    ...
    try {
      ...
      const newFood = await foodRepository.addFood({
        ...
        calories: numeric(manualForm.calories),
        protein: numeric(manualForm.protein),
        carbsTotal: numeric(manualForm.carbs),
        fatTotal: numeric(manualForm.fat),
        isGeneric: true, // It's a manual entry, so generic
      });

      if (!newFood || !newFood.id) {
        throw new Error('Impossibile creare l\'alimento');
      }

      // 2. Add the diary entry linked to the new food
      await addEntry({
        foodId: newFood.id, // Now we have a valid ID
        foodName: manualForm.name.trim(),
        consumedQuantity: servingSize,
        consumedUnit: manualForm.unit || 'g',
        calculatedNutrients: {
          calories: numeric(manualForm.calories),
          protein: numeric(manualForm.protein),
          carbohydrates_total: numeric(manualForm.carbs),
          fat_total: numeric(manualForm.fat)
        },
        consumptionDate: date,
        mealType: null,
      ...
      setManualForm({ name: '', quantity: '100', unit: 'g', calories: '', protein: '', carbs: '', fat: '' });
          ...
          <View style={styles.modalOverlay}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, justifyContent: 'center' }}>
              <View style={styles.modalCard}>
                <Text style={styles.modalTitle}>Aggiungi Manualmente</Text>
                <TextInput placeholder="Nome alimento" placeholderTextColor={colors.textMuted} style={styles.input} value={manualForm.name} onChangeText={t => setManualForm(f => ({ ...f, name: t }))} />
                <View style={styles.rowInputs}>
                  <TextInput placeholder="Quantità" placeholderTextColor={colors.textMuted} keyboardType="numeric" style={[styles.input, { flex: 1 }]} value={manualForm.quantity} onChangeText={t => setManualForm(f => ({ ...f, quantity: t }))} />
                  <TextInput placeholder="Unità (g)" placeholderTextColor={colors.textMuted} style={[styles.input, { width: 80, marginLeft: 10 }]} value={manualForm.unit} onChangeText={t => setManualForm(f => ({ ...f, unit: t }))} />
                </View>
                <Text style={[styles.sectionSub, { marginBottom: 8 }]}>Macronutrienti (opzionali)</Text>
                <View style={styles.rowInputs}>
                  <TextInput placeholder="Kcal" placeholderTextColor={colors.textMuted} keyboardType="numeric" style={[styles.input, { flex: 1 }]} value={manualForm.calories} onChangeText={t => setManualForm(f => ({ ...f, calories: t }))} />
                  <TextInput placeholder="Prot" placeholderTextColor={colors.textMuted} keyboardType="numeric" style={[styles.input, { flex: 1, marginLeft: 8 }]} value={manualForm.protein} onChangeText={t => setManualForm(f => ({ ...f, protein: t }))} />
                </View>
                <View style={styles.rowInputs}>
                  <TextInput placeholder="Carbo" placeholderTextColor={colors.textMuted} keyboardType="numeric" style={[styles.input, { flex: 1 }]} value={manualForm.carbs} onChangeText={t => setManualForm(f => ({ ...f, carbs: t }))} />
                  <TextInput placeholder="Grassi" placeholderTextColor={colors.textMuted} keyboardType="numeric" style={[styles.input, { flex: 1, marginLeft: 8 }]} value={manualForm.fat} onChangeText={t => setManualForm(f => ({ ...f, fat: t }))} />
                </View>
                <View style={styles.modalBtnsRow}>
                  <Pressable style={[styles.btn, styles.btnCancel]} onPress={() => setManualModal(false)}><Text style={styles.btnTxt}>Annulla</Text></Pressable>
                  <Pressable style={[styles.btn, styles.btnPrimary, !manualForm.name && styles.btnDisabled]} disabled={!manualForm.name || manualSubmitting} onPress={submitManual}>
                    <Text style={styles.btnPrimaryTxt}>{manualSubmitting ? '...' : 'Aggiungi'}</Text>
                  </Pressable>
                </View>
              </View>
